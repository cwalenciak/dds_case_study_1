...
ggplot(data = state_ibu_medians, aes(x=reorder(State,-IBU_Medians), y=IBU_Medians, width = .5,fill=State)) +  geom_bar(stat="identity") + labs(title="Median IBU", x="State", y = "IBU")  + plot1 + coord_flip() + 
...
ggplot(data = state_abv_medians, aes(x=reorder(State,-ABV_Medians), y=ABV_Medians, width = .5,fill=State)) +  geom_bar(stat="identity") + labs(title="Median ABV", x="State", y = "ABV")  + plot1 + coord_flip()
```
...
<br /> 
```{r q7}
scatterplot = ggscatter(na.omit(MergedBrewing),x="ABV",y="IBU",add="reg.line",conf.int="T",cor.coef="T",color = "blue",ellipse.type="norm",ellipse="F")+theme_minimal()+ stat_density_2d(aes(fill = ..level..), geom = "polygon")+scale_fill_continuous(low="lavenderblush", high="purple")
